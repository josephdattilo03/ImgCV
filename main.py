...
def start_video_stream(camera_idx):
    """Capture video and control MIDI note and speed based on brightness and motion."""
    ...
    while True:
        ...
        # *** Exponential mapping for drastic sleep change ***
        # Higher motion_level = much faster, even small motions matter
        min_sleep = 0.1
        max_sleep = 3.0
        normalized_motion = np.clip(motion_level / 50, 0, 1)  # 0–1 (motion around 50 already considered "lots")
        sleep_time = max_sleep * (1 - normalized_motion) ** 3  # Exponential falloff
        
        sleep_time = np.clip(sleep_time, min_sleep, max_sleep)  # Safety clamp

        print(f"midi_note: {midi_note}")
        print(f"brightness: {brightness:.2f}, motion_level: {motion_level:.2f}, sleep_time: {sleep_time:.2f}s")

        # Send MIDI messages
        note_on = mido.Message('note_on', note=int(midi_note), velocity=64)
        note_off = mido.Message('note_off', note=int(midi_note), velocity=64)
